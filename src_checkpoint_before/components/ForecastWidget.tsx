...
import { FIXED_FORECAST_THRESHOLDS, calculateOverallForecast } from '../utils/forecastEngine';
...
export const ForecastWidget: React.FC<ForecastWidgetProps> = ({
  ...
  const [dataScope, setDataScope] = useState<'cycle' | 'newest'>('cycle');
  ...
  const activeSummary = React.useMemo(() => {
    ...
    if (shouldScopeToSelectedDate || dataScope === 'newest') {
      return calculateOverallForecast(effectiveHistoryRecords, baseline, thresholds, dataScope);
    }
  ...
  return (
    <div className="bg-white rounded-md border border-slate-200 p-5 shadow-xs space-y-4">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pb-3 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2 flex-wrap">
              <span>Water Washing Early Warning Forecast</span>
              <span className="text-[10px] font-semibold bg-sky-50 text-sky-700 px-2 py-0.5 rounded border border-sky-200">
                Linear Regression D(t) = a + b·t
              </span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Sumber dataset: Tabel Input History harian untuk parameter PR, P3.0, NPHR, dan Real Power
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap self-start lg:self-auto">
          ...
          <div className="inline-flex rounded border border-slate-200 p-0.5 bg-slate-100 text-xs">
            <button
              type="button"
              onClick={() => setDataScope('cycle')}
              className={`px-2.5 py-1 rounded text-xs font-semibold transition-all ${
                dataScope === 'cycle'
                  ? 'bg-white text-slate-900 shadow-2xs'
                  : 'text-slate-600 hover:text-slate-900'
              }`}
              title="Gunakan seluruh data sejak baseline aktif"
            >
              Current Cycle {cycleStartDate ? `(${cycleStartDate})` : ''}
            </button>
            <button
              type="button"
              onClick={() => setDataScope('newest')}
              className={`px-2.5 py-1 rounded text-xs font-semibold transition-all ${
                dataScope === 'newest'
                  ? 'bg-white text-sky-700 shadow-2xs font-bold'
                  : 'text-slate-600 hover:text-slate-900'
              }`}
              title="Gunakan kluster data input terbaru di Input History"
            >
              Data Terbaru
            </button>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 rounded hover:bg-slate-100 transition-colors"
          >
            {isDetailsExpanded ? (
              <>
        ...
        <div className="p-3 bg-amber-50/90 border border-amber-300 rounded-md text-xs text-amber-950 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 shadow-2xs">
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              Pada titik ini, hanya parameter PR dan P3.0 yang telah melampaui batas ambang, sementara Real Power dan NPHR masih normal.
              Hasil forecast kini sepenuhnya konsisten dengan status <strong>Pantau / Monitoring (60%)</strong> di atas.
            </div>
          </div>
          <button
            type="button"
      ...
      {!hasBaseline ? (
        ...
        /* CASE 2: PROJECTED THRESHOLD CROSSING -> ~X HARI LAGI (OVERALL STATUS: NORMAL DALAM PEMANTAUAN) */
        <div className="p-4 rounded-md bg-gradient-to-r from-sky-50/95 via-blue-50/70 to-indigo-50/50 border border-sky-300 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start sm:items-center gap-3.5">
            <div className="w-14 h-14 rounded-full bg-sky-600 text-white flex flex-col items-center justify-center shrink-0 shadow-md font-black ring-4 ring-sky-200">
              <span className="text-base leading-none">~{forecastWW}</span>
              <span className="text-[9px] font-extrabold uppercase tracking-wider mt-0.5 text-sky-100">Hari</span>
            </div>
            <div>
              <div className="text-[11px] font-bold uppercase tracking-wider text-sky-800 flex items-center gap-2 flex-wrap">
                <span className="text-[10px] font-black bg-emerald-600 text-white px-2 py-0.5 rounded-full shadow-2xs">
                  STATUS: NORMAL
                </span>
                <span className="text-[10px] font-bold bg-sky-600 text-white px-2 py-0.5 rounded-full shadow-2xs">
                  REGRESI LINEAR AKTIF
                </span>
              </div>
              <div className="text-base sm:text-lg font-black text-slate-900 mt-0.5">
                Water Wash Selanjutnya: <span className="text-sky-700">~{forecastWW} Hari Lagi</span>
                {forecastWWDate && <span className="text-slate-600 font-semibold text-sm sm:text-base"> (Estimasi: {forecastWWDate})</span>}
              </div>
              <div className="text-xs text-slate-600 mt-1 flex flex-wrap items-center gap-x-2 gap-y-1">
                <span>
                  Formula: <strong className="text-slate-800 font-mono text-[11px]">MAX(PR, P3.0, MIN(NPHR, Real Power))</strong>
                </span>
                {governingParameter && (
                  <>
                    <span className="text-slate-400">•</span>
                    <span>
                      Penentu (Governing): <strong className="text-sky-800 font-bold">{getParamLabel(governingParameter)}</strong>
                    </span>
                  </>
                )}
                <span className="text-slate-400">•</span>
              ...
              {reachedParameters.length > 0 && (
                <div className="mt-2 text-[11px] text-amber-800 bg-amber-50/90 border border-amber-200 px-2.5 py-1 rounded inline-flex items-center gap-1.5 flex-wrap">
                  <span className="font-bold">Info:</span>
                  <span>
                    Parameter <strong>{reachedParameters.map((p) => getParamLabel(p)).join(', ')}</strong> telah mencapai ambang batas (dihitung 0 hari), namun status Water Wash tetap <strong>Normal</strong> karena menunggu kondisi <em>PR AND P3.0 AND (NPHR OR Real Power)</em> yang diproyeksikan tercapai dalam <strong>~{forecastWW} hari</strong>.
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0 self-start md:self-auto">
            <div className="flex items-center gap-2.5 text-xs bg-white/95 border border-sky-200 px-3.5 py-2.5 rounded shadow-2xs">
              ...
              <div>
                <span className="font-semibold text-slate-900">Rekomendasi Tindakan:</span>{' '}
                {forecastWW <= 4 ? (
                  <span className="text-rose-700 font-bold">Siapkan jadwal Off-line WW</span>
                ) : forecastWW <= 7 ? (
                  <span className="text-amber-700 font-semibold">Pantau tren harian secara ketat</span>
                ) : (
        ...
        <div className="p-4 bg-amber-50/90 border border-amber-300 rounded text-xs text-amber-950 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start sm:items-center gap-3.5">
            <div className="w-14 h-14 rounded-full bg-amber-600 text-white flex flex-col items-center justify-center shrink-0 shadow-md font-black ring-4 ring-amber-200">
              <span className="text-base leading-none">{dailyRecords.length}/4</span>
            ...
            <div>
              ...
              <div className="text-base font-extrabold text-amber-950 mt-0.5">
                Water Wash Selanjutnya: <span className="text-amber-800">Menunggu Tambahan {4 - dailyRecords.length} Data Harian</span>
              </div>
              <p className="text-amber-800 text-xs mt-1">
                Dibutuhkan minimal 4 data harian pada siklus ini untuk menghitung regresi linear secara andal. Saat ini baru tersedia {dailyRecords.length} data harian.
              </p>
            </div>
          </div>
          <span className="text-[11px] font-mono font-bold bg-white text-amber-800 border border-amber-200 px-3 py-1.5 rounded shrink-0 self-start md:self-auto">
            {dailyRecords.length} / 4 Observasi
          </span>
        </div>
      ) : (
      ...
      {/* Formula & Parameter Breakdown Strip */}
      <div className="p-3 bg-slate-50 border border-slate-200 rounded text-xs space-y-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 font-bold text-slate-800">
            <Calculator className="w-4 h-4 text-sky-600" />
            <span>Kombinasi Logika Water Wash:</span>
            <span className="font-mono text-[11px] bg-white border border-slate-200 px-2 py-0.5 rounded text-slate-700">
              Forecast WW = MAX(PR, P3.0, MIN(NPHR, Real Power))
            </span>
          </div>
          <div className="text-[11px] text-slate-500 flex items-center gap-1.5 flex-wrap">
            {shouldScopeToSelectedDate && (
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 font-semibold text-[10px]">
                <Clock className="w-2.5 h-2.5" /> Dikalibrasi s.d. {selectedDate}
              </span>
            )}
            <span>
              Sumber Data: <strong>Daily Median Input History</strong> ({dailyRecords.length} hari dari baseline)
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
          <div className="bg-white p-2 rounded border border-slate-200">
            <div className="flex items-center justify-between text-[11px] text-slate-500 font-medium">
              <span>PR (Ambang 2.5%)</span>
              {governingParameter === 'PR' && (
                <span className="text-[9px] font-black bg-sky-100 text-sky-800 px-1 py-0.2 rounded">Penentu</span>
              )}
            </div>
            <div className="font-bold text-slate-900 mt-0.5">
              {forecastDaysPR === 0 ? (
                <span className="text-rose-600 font-black">0 Hari (Reached)</span>
              ) : forecastDaysPR !== null ? (
                <span className="text-sky-700 font-extrabold">~{forecastDaysPR} Hari</span>
              ) : (
                <span className="text-slate-500">Stabil (&gt;30d)</span>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-2 rounded border border-slate-200">
            <div className="flex items-center justify-between text-[11px] text-slate-500 font-medium">
              <span>P3.0 (Ambang 2.5%)</span>
              {governingParameter === 'P3_0' && (
                <span className="text-[9px] font-black bg-sky-100 text-sky-800 px-1 py-0.2 rounded">Penentu</span>
              )}
            </div>
            <div className="font-bold text-slate-900 mt-0.5">
              {forecastDaysP3 === 0 ? (
                ...
                <span className="text-sky-700 font-extrabold">~{forecastDaysP3} Hari</span>
              ) : (
                <span className="text-slate-500">Stabil (&gt;30d)</span>
              )}
            </div>
          ...
          <div className="bg-white p-2 rounded border border-slate-200">
            <div className="flex items-center justify-between text-[11px] text-slate-500 font-medium">
              <span>MIN(NPHR, Power)</span>
              {(governingParameter === 'NPHR' || governingParameter === 'realPower') && (
                <span className="text-[9px] font-black bg-sky-100 text-sky-800 px-1 py-0.2 rounded">Penentu</span>
              )}
            </div>
            <div className="font-bold text-slate-900 mt-0.5">
              {forecastDaysAux === 0 ? (
                <span className="text-rose-600 font-black">0 Hari (Reached)</span>
              ) : forecastDaysAux !== null ? (
                <span className="text-sky-700 font-extrabold">~{forecastDaysAux} Hari</span>
              ) : (
                <span className="text-slate-500">Stabil (&gt;30d)</span>
              )}
            </div>
            <div className="text-[10px] text-slate-500">
              NPHR: {forecastDaysNPHR === 0 ? '0d' : forecastDaysNPHR !== null ? `${forecastDaysNPHR}d` : '—'} | Power: {forecastDaysPower === 0 ? '0d' : forecastDaysPower !== null ? `${forecastDaysPower}d` : '—'}
            </div>
          </div>

          <div className="bg-sky-50/80 p-2 rounded border border-sky-200">
            <div className="text-[11px] text-sky-800 font-bold">Hasil Kombinasi WW</div>
            <div className="font-black text-sky-900 text-sm mt-0.5">
              {forecastWW === 0 ? (
                <span className="text-rose-700">0 Hari (Due Now)</span>
              ) : forecastWW !== null ? (
                <span>~{forecastWW} Hari Lagi</span>
              ) : (
                <span className="text-emerald-700">&gt;30 Hari Lagi</span>
              )}
            </div>
            <div className="text-[10px] text-sky-700">
              {waterWashStatus === 'DUE_NOW'
                ...
                ? `Status: Normal (menunggu kriteria lengkap)`
                : 'Kondisi stabil'}
            </div>
          </div>
        </div>
        ...
        {selectedParamChart === 'ALL' ? (
          <div className="flex items-center justify-between text-xs px-1 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2 text-slate-500 text-[11px] flex-wrap">
              ...
              <span>Ambang Batas: PR (2.5%), P3.0 (2.5%), NPHR (3.0%), Real Power (4.0%)</span>
            </div>
          </div>
        ) : (
          <div className="flex items-center justify-between text-xs px-1 flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {activeFc.status === 'THRESHOLD REACHED' ? (
                ...
                <span className="text-amber-700 italic flex items-center gap-1">
                  ...
                  Menunggu data observasi ({activeFc.validDaysCount}/4 data tersedia)
                </span>
              )}
            </div>
        ...
        {selectedParamChart === 'ALL' ? (
          allViewMode === 'combined' ? (
            <div className="space-y-3">
              ...
              {/* 4-Parameter Summary Matrix Strip under Combined Chart */}
                ...
                {(['PR', 'P3_0', 'NPHR', 'realPower'] as const).map((param) => {
                  ...
                  return (
                    <div
                      key={param}
                      onClick={() => setSelectedParamChart(param)}
                      className="p-2.5 rounded border border-slate-200 bg-white hover:border-sky-300 hover:shadow-xs cursor-pointer transition-all flex flex-col justify-between"
                    >
                      ...
                      <div className="text-[10px] text-slate-400 hover:text-sky-600 text-right mt-1">
                        Klik untuk grafik detail →
                      </div>
                    </div>
                  );
                })}
      ...
      {/* Parameter Breakdown Grid across all 4 parameters (Requirement 1, 12, 14, 15) */}
      {isDetailsExpanded && (
        <div className="pt-3 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {(['PR', 'P3_0', 'NPHR', 'realPower'] as const).map((param) => {
            const fc = parameters[param];
            const isNearest = nearestParameter === param && hasForecast;

            return (
              <div
                key={param}
                className={`p-3 rounded-md border text-xs space-y-1.5 transition-colors ${
                  isNearest
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {isNearest && (
                    <span className="text-[10px] font-bold bg-sky-600 text-white px-1.5 py-0.2 rounded">
                      Nearest
                    </span>
                  )}
                  {fc.status === 'THRESHOLD REACHED' && (
                ...
                <div className="text-slate-600 flex justify-between">
                  <span>Current Det.:</span>
                  <strong className="text-slate-900 font-semibold">
                    {fc.currentDeterioration !== null ? `${fc.currentDeterioration}%` : 'N/A'}
                  </strong>
                </div>

                <div className="text-slate-600 flex justify-between">
                  <span>Fixed Limit:</span>
                  <span className="font-semibold text-rose-700">{fc.threshold}%</span>
                </div>
                ...
                <div className="text-slate-600 flex justify-between">
                  ...
                  <strong
                    className={
                      fc.remainingDeterioration && fc.remainingDeterioration > 0
                        ? 'text-slate-800'
                        : 'text-rose-600'
                    }
                  >
                    {fc.remainingDeterioration !== null ? `${fc.remainingDeterioration} pp` : 'N/A'}
                  </strong>
                </div>

                <div className="text-slate-600 flex justify-between border-t border-slate-200 pt-1">
                  <span>Slope b (rate):</span>
                  <span>{fc.slope !== null ? `${fc.slope > 0 ? '+' : ''}${fc.slope} %/d` : '—'}</span>
                </div>

                <div className="text-slate-600 flex justify-between font-semibold">
                  <span>Forecast WW:</span>
                  {fc.status === 'THRESHOLD REACHED' ? (
                    <span className="text-rose-700 font-black">0 Hari Lagi (Due Now)</span>
                  ) : fc.status === 'FORECAST AVAILABLE' && fc.daysToThreshold !== null ? (
                    <strong className="text-sky-700 font-extrabold">
                      ~{fc.daysToThreshold} Hari Lagi
                    </strong>
                  ) : fc.status === 'NO PROJECTED THRESHOLD CROSSING' ? (
                    <span className="text-emerald-700 font-medium">&gt;30 Hari (Stabil)</span>
                  ) : (
                    <span className="text-amber-700 italic">Perlu Data</span>
                  )}
                </div>

                {fc.estimatedDate && fc.status === 'FORECAST AVAILABLE' && (
                  <div className="text-[11px] text-slate-500 text-right pt-0.5">
                    Est. date: <strong>{fc.estimatedDate}</strong>
                  </div>
                )}
              </div>
            );
